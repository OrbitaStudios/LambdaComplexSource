...
void CVPC::SetMacrosAndConditionals()
{
	...
	if ( !pPlatformConditional )
	{
		...
		pPlatformConditional = FindOrCreateConditional( "EMSCRIPTEN", false, CONDITIONAL_PLATFORM );
#else
#error "Unsupported platform."
	...
	else if ( V_stricmp( cVPCPlatform.String(), "EMSCRIPTEN" ) == 0)
	{
		...
		// Maybe we should use WebGL, but currently I don't know shit about WebGL, so we are going to use OpenGL
	#if !defined(SOURCE_WEBGL)
		SetConditional( "GL" );
	#else
...
#else
	bool bUseMakefile = bIsEmscripten || ( pConditional && pConditional->m_bDefined );
#endif	
...
#else
		Log_Msg( LOG_VPC, "\n** Detected Emscripten platform. Using Emscripten Base Makefile.\n" );
#endif
	}
